...
# 배경 만들기
background = pygame.image.load(os.path.join(image_path, "background.jpg")) # 전체경로를 적지 않아도 됨
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        if ball_pos_x < 0 or ball_pos_x > screen_width - ball_width: 
            ball_val["to_x"] = ball_val["to_x"] * -1 # 공의 이동방향을 반대로 바꿈
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    # 나눠진 공 정보
                    small_ball_rect = ball_images[ball_img_idx + 1].get_rect() # 다음 인덱스 공 사진을 불러오기 위해 +1을 함.
                    ...
                    balls.append({
                        ...
                        "to_x" : -3, # x축 이동방향, -3 이면 왼쪽으로, 3 이면 오른쪽으로
                        "to_y" : -6, # y축 이동방향,
                        "init_spd_y" : ball_speed_y[ball_img_idx + 1]}) # 쪼개진 공의 속도
        ...
        break # 안쪽 for문에서 break를 만나면 여기로 진입. 2중 for문을 한 번에 탈출
